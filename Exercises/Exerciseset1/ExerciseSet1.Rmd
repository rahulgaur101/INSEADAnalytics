...
**Your Answers here:
1.mytickers = c("SPY", "AAPL") is defined in the dataSet1 and contains the Apple ticker which results in the related data being downloaded becuase SPY points to the Dataset1 via helpersSet1
...
**Your Answers here:
1. transposedData = t(StockReturns)
nrow(transposedData)
ncol(transposedData)
...
new = apply(transposedData,2,mean)
names(new) <- colnames(transposedData)
pnl_plot(new)
...
1. Change start date to "2001-01-02"
<br>
**
<br>
<br>
