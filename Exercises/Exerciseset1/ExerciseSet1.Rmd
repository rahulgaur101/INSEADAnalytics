...
1.mytickers = c("SPY", "AAPL") is defined in the dataSet1 and contains the Apple ticker which results in the related data being downloaded becuase SPY points to the Dataset1 via helpersSet1 <br>


2. The results for Apple:
cumulative: 595.5%
average daily returns: 0.148%
sd: 2.39% <br>


3. Change dataSet1.R line 8 to:
mytickers = c("SPY", "AAPL", "YHOO") and then run the data file. Use the same commoands to get the data for returns except that this time point the command to the column for Yahoo 

round(100*sum(StockReturns[,3]),1)<br>
Cumulative 291.3<br>
round(100*mean(StockReturns[,3]),3)<br>
Average Daily 0.072<br>
round(100*sd(StockReturns[,3]),3)<br>
sd 3.02
 <br>
...
**Your Answers here:<br>
1. transposedData = t(StockReturns)<br>
nrow(transposedData)<br>
ncol(transposedData)

<br>

2. transposedData = t(StockReturns)<br>
new = apply(transposedData,2,mean)<br>
names(new) <- colnames(transposedData)<br>
pnl_plot(new)
...
**Your Answers here:<br>
1. Change start date to "2000-12-29 as that was the end of year 2000. Doing so would give you the data from the first day of 2001"
<br>
**
<br>
<br>
...
**Your Answers here:
1. I get the sum of the stock returns for 20 rows
<br>
2. I see an error "non numeric argument to binary operator" as there is a mismatch in data formats and the number of rows
**
<br>
<br>
<br>
